...
//======================================================================
    ...
    private function lvaycXcampPRA($tableFb, $skillDefFb, $fechaStart, $fechaEnd)
    {
        ...
        SUM(CASE WHEN (lc.resultdesc = 'COTIZACION' OR lc.resultdesc = 'VENTA' OR lc.resultdesc = 'PREVENTA') THEN 1 ELSE 0 END) AS ventas,
        -- SUM(CASE WHEN lc.resultdesc = 'COTIZACION' AND lc.skilldata <> 'FB_UIMotor' THEN 1 ELSE 0 END) AS llamadasVentasGo,
        ((SUM(CASE WHEN (lc.resultdesc = 'COTIZACION' OR lc.resultdesc = 'VENTA' OR lc.resultdesc = 'PREVENTA')  THEN 1 ELSE 0 END) / ca.primerContacto) * 100 ) AS Ratio
                     ...
                     INNER JOIN ocmdb.".$tableFb."exten de ON d.id = de.id
                                 WHERE d.dateinsert BETWEEN ".$fechaStart." AND ".$fechaEnd."
                                    AND lc.skilldata IN ('".$skillDefFb."')
                                      AND de.id_lead <> ''
                                      AND endresultdesc <> 'Time Rule Deny' AND endresultdesc <> 'Abandono' AND endresultdesc <> 'Dial Error' AND endresultdesc <> 'Fin'
                                 GROUP BY lc.idreg )
